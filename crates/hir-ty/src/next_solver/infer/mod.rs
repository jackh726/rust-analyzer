...
use super::{
    ...
    Predicate, Region, SolverDefId, Span, SubtypePredicate, Term, TraitPredicate, TraitRef, Ty,
    TyKind, TypingMode,
};
...
#[derive(Clone, Debug)]
pub enum SubregionOrigin<'db> {
    /// Arose from a subtyping relation
    Subtype(Box<TypeTrace<'db>>),
    ...
    RelateParamBound(Span, Ty<'db>, Option<Span>),

    /// The given region parameter was instantiated with a region
    /// that must outlive some other region.
    RelateRegionParamBound(Span, Option<Ty<'db>>),

    /// Creating a pointer `b` to contents of another reference.
    Reborrow(Span),

    /// (&'a &'b T) where a >= b
    ReferenceOutlivesReferent(Ty<'db>, Span),

    /// Comparing the signature and requirements of an impl method against
    /// the containing trait.
    CompareImplItemObligation {
        span: Span,
        impl_item_def_id: SolverDefId,
        trait_item_def_id: SolverDefId,
    },

    /// Checking that the bounds of a trait's associated type hold for a given impl.
    CheckAssociatedTypeBounds {
        parent: Box<SubregionOrigin<'db>>,
        impl_item_def_id: SolverDefId,
        trait_item_def_id: SolverDefId,
    },
...
impl<'db> InferCtxt<'db> {
    ...
    pub fn next_ty_var_in_universe(&self, span: Span, universe: UniverseIndex) -> Ty<'db> {
    ...
    pub fn next_const_var_in_universe(&self, span: Span, universe: UniverseIndex) -> Const<'db> {
    ...
    pub fn next_region_var(&self, origin: RegionVariableOrigin) -> Region<'db> {
    ...
    #[instrument(skip(self), level = "debug")]
    pub fn next_nll_region_var(&self, origin: NllRegionVariableOrigin) -> Region<'db> {
    ...
    fn var_for_def(&self, span: Span, kind: GenericParamDefKind, name: &Symbol) -> GenericArg<'db> {
        match kind {
    ...
    pub fn get_region_var_origins(&self) -> VarInfos {
        ...
        assert!(!UndoLogs::<UndoLog<'db>>::in_snapshot(&inner.undo_log));
